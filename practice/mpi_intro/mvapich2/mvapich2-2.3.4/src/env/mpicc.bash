...
if [ "$linking" = yes ] ; then
    ...
    if [ "$nativelinking" = yes ] ; then
        ...
        $Show $CC ${final_cppflags} $PROFILE_INCPATHS ${final_cflags} -lmpi ${final_ldflags} "${allargs[@]}" -I$includedir $ITAC_OPTIONS -L$libdir $PROFILE_PRELIB $PROFILE_FOO $rpath_flags  $PROFILE_POSTLIB ${final_libs}
        rc=$?
    fi
else
