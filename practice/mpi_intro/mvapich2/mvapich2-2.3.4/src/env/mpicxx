#! /usr/bin/bash
#
# Copyright (c) 2001-2020, The Ohio State University. All rights reserved.
#  
...
if test "yes" = "no" -o "${interlib_deps}" = "no" ; then
    ...
    final_libs="${final_libs} -lm -lpthread    -lrt -lm -lpthread "
fi
...
if [ "$linking" = yes ] ; then
    ...
    if [ "$nativelinking" = yes ] ; then
        ...
        $Show $CXX ${final_cppflags} $PROFILE_INCPATHS ${final_cxxflags} $cxxlibs -lmpi ${final_ldflags} "${allargs[@]}" -I$includedir $ITAC_OPTIONS -L$libdir $PROFILE_PRELIB $PROFILE_FOO $rpath_flags  $PROFILE_POSTLIB ${final_libs}
        rc=$?
    fi
else
