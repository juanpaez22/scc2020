...
if [ "$linking" = yes ] ; then
    ...
    if [ "$nativelinking" = yes ] ; then
        ...
        $Show $FC $PROFILE_INCPATHS ${final_fcflags} -lmpifort -lmpi ${final_ldflags} $allargs $FCINCDIRS $FCMODDIRS $ITAC_OPTIONS -L$libdir $PROFILE_PRELIB $PROFILE_FOO $rpath_flags  $PROFILE_POSTLIB ${final_libs} 
        rc=$?
    fi
else
